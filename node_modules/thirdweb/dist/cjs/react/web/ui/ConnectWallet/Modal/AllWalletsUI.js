"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const jsx_runtime_1 = require("react/jsx-runtime");
const react_1 = require("react");
const Spacer_js_1 = require("../../components/Spacer.js");
const basic_js_1 = require("../../components/basic.js");
const formElements_js_1 = require("../../components/formElements.js");
const useDebouncedValue_js_1 = require("../../hooks/useDebouncedValue.js");
const Spinner_js_1 = require("../../components/Spinner.js");
const index_js_1 = require("../../design-system/index.js");
const CustomThemeProvider_js_1 = require("../../design-system/CustomThemeProvider.js");
const styled_1 = require("@emotion/styled");
const react_icons_1 = require("@radix-ui/react-icons");
const WalletEntryButton_js_1 = require("../WalletEntryButton.js");
const create_wallet_js_1 = require("../../../../../wallets/create-wallet.js");
const useShowMore_js_1 = require("../../hooks/useShowMore.js");
const sortWallets_js_1 = require("../../../utils/sortWallets.js");
const useWalletConnectionCtx_js_1 = require("../../../../core/hooks/others/useWalletConnectionCtx.js");
const wallet_ui_states_provider_js_1 = require("../../../providers/wallet-ui-states-provider.js");
const fuse_js_1 = require("fuse.js");
const wallet_infos_js_1 = require("../../../../../wallets/__generated__/wallet-infos.js");
const text_js_1 = require("../../components/text.js");
const react_icons_2 = require("@radix-ui/react-icons");
/**
 *
 * @internal
 */
function AllWalletsUI(props) {
    const { recommendedWallets, wallets: specifiedWallets } = (0, useWalletConnectionCtx_js_1.useWalletConnectionCtx)();
    const { modalSize } = (0, react_1.useContext)(wallet_ui_states_provider_js_1.ModalConfigCtx);
    const fuseInstance = (0, react_1.useMemo)(() => {
        return new fuse_js_1.default(wallet_infos_js_1.default, {
            threshold: 0.4,
            keys: [
                {
                    name: "name",
                    weight: 1,
                },
            ],
        });
    }, []);
    const listContainer = (0, react_1.useRef)(null);
    const [searchTerm, setSearchTerm] = (0, react_1.useState)("");
    const deferredSearchTerm = (0, useDebouncedValue_js_1.useDebouncedValue)(searchTerm, 300);
    const walletInfosWithSearch = deferredSearchTerm
        ? fuseInstance.search(deferredSearchTerm).map((result) => result.item)
        : wallet_infos_js_1.default;
    const installedWalletsFirst = (0, sortWallets_js_1.sortWallets)(walletInfosWithSearch, recommendedWallets);
    // show specified wallets first
    const sortedWallets = (0, react_1.useMemo)(() => {
        return installedWalletsFirst.sort((a, b) => {
            const aIsSpecified = specifiedWallets.find((w) => w.id === a.id);
            const bIsSpecified = specifiedWallets.find((w) => w.id === b.id);
            if (aIsSpecified && !bIsSpecified) {
                return -1;
            }
            if (!aIsSpecified && bIsSpecified) {
                return 1;
            }
            return 0;
        });
    }, [installedWalletsFirst, specifiedWallets]);
    const { itemsToShow, lastItemRef } = (0, useShowMore_js_1.useShowMore)(10, 10);
    const walletInfosToShow = sortedWallets.slice(0, itemsToShow);
    return ((0, jsx_runtime_1.jsxs)(basic_js_1.Container, { fullHeight: true, flex: "column", animate: "fadein", children: [(0, jsx_runtime_1.jsx)(basic_js_1.Container, { p: "lg", children: (0, jsx_runtime_1.jsx)(basic_js_1.ModalHeader, { title: "Select Wallet", onBack: props.onBack }) }), (0, jsx_runtime_1.jsx)(Spacer_js_1.Spacer, { y: "xs" }), (0, jsx_runtime_1.jsx)(basic_js_1.Container, { px: "lg", children: (0, jsx_runtime_1.jsxs)("div", { style: {
                        display: "flex",
                        alignItems: "center",
                        position: "relative",
                    }, children: [(0, jsx_runtime_1.jsx)(StyledMagnifyingGlassIcon, { width: index_js_1.iconSize.md, height: index_js_1.iconSize.md }), (0, jsx_runtime_1.jsx)(formElements_js_1.Input, { style: {
                                padding: `${index_js_1.spacing.sm} ${index_js_1.spacing.md} ${index_js_1.spacing.sm} ${index_js_1.spacing.xxl}`,
                            }, tabIndex: -1, variant: "outline", placeholder: "Search Wallet", value: searchTerm, onChange: (e) => {
                                listContainer.current?.parentElement?.scroll({
                                    top: 0,
                                });
                                setSearchTerm(e.target.value);
                            } }), deferredSearchTerm !== searchTerm && ((0, jsx_runtime_1.jsx)("div", { style: {
                                position: "absolute",
                                right: index_js_1.spacing.md,
                            }, children: (0, jsx_runtime_1.jsx)(Spinner_js_1.Spinner, { size: "md", color: "accentText" }) }))] }) }), walletInfosToShow.length > 0 && ((0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [(0, jsx_runtime_1.jsx)(Spacer_js_1.Spacer, { y: "md" }), (0, jsx_runtime_1.jsxs)(basic_js_1.Container, { animate: "fadein", expand: true, scrollY: true, children: [(0, jsx_runtime_1.jsx)("div", { ref: listContainer, style: {
                                    maxHeight: modalSize === "compact" ? "400px" : undefined,
                                    paddingInline: index_js_1.spacing.md,
                                }, children: walletInfosToShow.map((walletInfo, i) => {
                                    const isLast = i === walletInfosToShow.length - 1;
                                    return ((0, jsx_runtime_1.jsx)("li", { ref: isLast ? lastItemRef : undefined, style: {
                                            listStyle: "none",
                                        }, children: (0, jsx_runtime_1.jsx)(WalletEntryButton_js_1.WalletEntryButton, { walletId: walletInfo.id, selectWallet: () => {
                                                const wallet = (0, create_wallet_js_1.createWallet)(walletInfo.id);
                                                props.onSelect(wallet);
                                            } }) }, walletInfo.id));
                                }) }), (0, jsx_runtime_1.jsx)(Spacer_js_1.Spacer, { y: "xl" })] })] })), walletInfosToShow.length === 0 && ((0, jsx_runtime_1.jsxs)(basic_js_1.Container, { flex: "column", gap: "md", center: "both", color: "secondaryText", animate: "fadein", expand: true, style: {
                    minHeight: "250px",
                }, children: [(0, jsx_runtime_1.jsx)(react_icons_2.CrossCircledIcon, { width: index_js_1.iconSize.xl, height: index_js_1.iconSize.xl }), (0, jsx_runtime_1.jsx)(text_js_1.Text, { children: " No Results " })] }))] }));
}
const StyledMagnifyingGlassIcon = /* @__PURE__ */ (0, styled_1.default)(react_icons_1.MagnifyingGlassIcon)(() => {
    const theme = (0, CustomThemeProvider_js_1.useCustomTheme)();
    return {
        color: theme.colors.secondaryText,
        position: "absolute",
        left: index_js_1.spacing.sm,
    };
});
exports.default = AllWalletsUI;
//# sourceMappingURL=AllWalletsUI.js.map