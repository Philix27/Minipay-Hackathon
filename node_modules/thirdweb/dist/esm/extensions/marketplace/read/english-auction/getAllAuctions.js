...
export async function getAllAuctions(options) {
    ...
    const [rawAuctions, latestBlock] = await Promise.all([
        getAllInBatches((startId, endId) => getAllAuctionsGenerated({ contract: options.contract, startId, endId }), {
            start,
            end,
    ...
    return await Promise.all(rawAuctions.map((rawAuction) => mapEnglishAuction({
