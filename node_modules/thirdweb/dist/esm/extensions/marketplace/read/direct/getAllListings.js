...
export async function getAllListings(options) {
    ...
    const [rawListings, latestBlock] = await Promise.all([
        getAllInBatches((startId, endId) => getAllListingGenerated({ contract: options.contract, startId, endId }), {
            start,
            end,
    ...
    return await Promise.all(rawListings.map((rawListing) => mapDirectListing({
